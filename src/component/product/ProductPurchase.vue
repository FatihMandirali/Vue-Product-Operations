...
<script>
  export default {
    ...
    computed: {
      saveEnabled() {
        if (this.product.description.length > 0 && this.product.count > 0 && this.product.price > 0 && this.product.title.length > 0)
          return false;
        return true;
      },
      isLoading(){
    ...
    beforeRouteLeave(to, from, next) {
      if((this.product.description.length > 0 || this.product.count > 0 || this.product.price > 0 || this.product.title.length > 0) && !this.saveButtonClick) {
        if(confirm("Kaydedilmemiş değişiklikler var. Yinede çıkmak istiyor musun ?"))
          next();
        else
          next(false);
      }
